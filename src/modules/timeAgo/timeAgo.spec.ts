import timeAgo, { checkFormatDateTime } from "./index";
...
describe("timeAgo", () => {
	it("Previous", () => {
		...
		expect(timeAgo(getTime(-5 * 24 * 60 * 60 * 1000))).toEqual(
			"حدود 5 روز قبل",
		);
		expect(timeAgo(getTime(-7 * 24 * 60 * 60 * 1000))).toEqual(
			"حدود 1 هفته قبل",
		);
		expect(timeAgo(getTime(-7 * 30 * 24 * 60 * 60 * 1000))).toEqual(
			"حدود 7 ماه قبل",
		);
		expect(timeAgo(getTime(-14 * 30 * 24 * 60 * 60 * 1000))).toEqual(
			"حدود 1 سال قبل",
		);
	});
