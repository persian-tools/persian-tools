...
class persianSwear {
	...
	addWord(word: string): void {
		const found = this.badWords.find((w) => w === word);
		if (found === undefined) this.badWords.push(word);
	}
	...
	isContain(text: string): boolean {
		for (let index = 0; index < this.badWords.length; index++) if (text.includes(this.badWords[index])) return true;
		return false;
	}
	...
	filterSwear(text: string, prefix = "***"): string {
		...
		for (let i = 0; i < text_splited.length; i++) {
			if (this.isBad(text_splited[i])) text_result += prefix;
